...
\chapter{Vytvoøení sítì pro simulaci v~SUMO}
\label{chap:netSUMO}
...
\chapter{Vytvoøení souborù pro nový model}
\label{chap:newfiles}
...
\chapter{$\chi^2$ test dobré shody}
\label{chap:chi2test}
Je všeobecnì známým faktem, že vozidla se generují z exponenciálního rozdìlení za podmínky, že èetnosti poètu vozidel, která vjíždí do oblasti, mají Poissonovo rozdìlení. Bylo proto nutné tento pøedpoklad otestovat. Pro toto testování se používá $\chi^2$ test dobré shody.

Statistika, podle které budeme testovat, má tvar
\begin{equation}
\chi^2 = \sum_{i=1}^k \frac{\left(x_i -n_i\right)^2}{n_i},
\label{eq:chi2}
\end{equation}
kde je $x_i$ skuteèná èetnost a $n_i$ oèekávaná èetnost dle Poissonova rozdìlení.

Pro tento test jsem zvolil dvì èásti dne -- ranní hodinu od 6:00 do 7:00 a odpolední hodinu mezi 15:00 a 18:00 hodinou v den státního svátku 28.10.2012, a to z dùvodu, že tento den jsem posléze simuloval provoz na Silnièním okruhu kolem Prahy.

$\chi^2$ rozdìlení má pouze jeden parametr $\chi^2 (n-1)$, kde $n$ je poèet stupòù volnosti. V našem pøípadì je n rovno poètu tøíd (øádkù v tabulkách -- viz dále).

V ranní špièce byly zjištìny hodnoty, které jsou shrnuty v tabulce \ref{tablerano}. 

\begin{table}[h]
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
Poèet vozidel & Èetnost & Oèekávaná èetnost \\ \hline
0             & 1       & 2,56             \\ \hline
1             & 9       & 9,6             \\ \hline
2             & 25      & 18            \\ \hline
3             & 20      & 22,52           \\ \hline
...
8             & 4       & 4,11            \\ \hline
\end{tabular}
\caption{Èetnosti v èase od 6:00 do 7:00, 28.10.2012}
\label{tablerano}
\end{center}
\end{table}

Stanovíme si hypotézu $H_0$, že výbìr vozidel pochází z Poissonova rozdìlení se støední hodnotou $\mu = 3{,}75$, kde støední hodnota byla zjištìna jako vážený prùmìr poètu èetností. Z èetností a oèekávaných èetností byla vypoètena statistika $\chi^2$,
\begin{equation}
\chi^2 = 7,56,
\label{eq:ci2stat1}
\end{equation}
pøièemž kritická hodnota pro $\chi^2$ na hladinì významnosti 5\,\% byla rovna
\begin{equation}
\chi^2_{\text{krit};0,95;8} = 15,51.
\label{eq:ci2krit1}
\end{equation} 

Z výše uvedených údajù je vidìt, že $\chi^2_{\text{krit};0,95;8} > \chi^2$ a s 5\% pravdìpodobností nezamítáme hypotézu, že se jedná o Poissonovo rozdìlení.

To samé provedeme pro druhou èást dne mezi 15:00 a 18:00. Èetnosti jsou shrnuty v tabulce \ref{tableodpo}.
...
40            & 13       & 7,96               \\ \hline
41            & 8        & 6,58               \\ \hline
42            & 25       & 16,37              \\ \hline\end{tabular}
\caption{Èetnosti v èase od 6:00 do 7:00, 28.10.2012}
\label{tableodpo}
\end{center}
\end{table}

Stanovíme si hypotézu $H_0$, že výbìr vozidel pochází z Poissonova rozdìlení se støední hodnotou $\mu = 33,91$. Z èetností a oèekávaných èetností byla vypoètena statistika $\chi^2$,
\begin{equation}
\chi^2 = 27,23,
\label{eq:ci2stat2}
\end{equation}
pøièemž kritická hodnota pro $\chi^2$ na hladinì významnosti 5\,\% byla rovna
\begin{equation}
\chi^2_{\text{krit};0,95;8} = 27,58.
\label{eq:ci2krit2}
\end{equation} 

Z výše uvedených údajù je vidìt, že $\chi^2_{\text{krit};0,95;8} > \chi^2$ a z 5\% pravdìpodobností nezamítáme hypotézu, že se jedná o Poissonovo rozdìlení.