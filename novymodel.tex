...
V simulaèním nástroji SUMO jsou již nìkteré z modelù uvedených v kapitole \ref{chap:predIDM} implementovány. 
...
V další fázi je potøeba tyto funkce využít pro výpoèty. Všechny následující funkce budeme pouze nahrazovat již stávajícími, které obsahuje \textit{MSCFModel}. Vždycky je otázkou, zda je nutné danou funkci pøepisovat a nebo nechat její stávající verzi.

Zaèneme funkcí \textit{freeSpeed}. Tato funkce nám vypoèítává rychlost pøi volném proudu, tj. ve chvíli kdy pøed aktuálním vozidlem nejede žádné další. Mohli bychom tuto funkci nechat stejnou, tak jak je implementována v SUMO, ovšem daleko lepší bude tuto funkci pøepsat a to z toho dùvodu, že Gippsùv model pøímo uvažuje o rychlosti pøi volném proudu. Definice pøepisovaných funkcí necháme vždy stejné jako v \textit{MSCFModel.h}, pro \textit{freeSpeed} by definice\footnote{Každá definice zde zmiòována bude umístìna do souboru MSCFModel\_Gipps.h do èásti public. Tato definice je shodná s definicí uvedenou v souboru MSCFModel.h} vypadala takto:
...
Je vidìt, že vìtšina vstupních parametrù funkce nejsou vùbec využity, nicménì musejí zde zùstat z dùvodu kompatibility s ostatními modely. Gippsùv model pøi výpoètu jednoduše tyto parametry jednoduše neøeší.

Další funkcí, kterou je nutné pøepsat, bude zcela jistì \textit{followSpeed}. Ta poèítá rychlost v závislosti na vedoucím vozidle. My již víme, že tato funkce by mìla vracet stejný výsledek jako rovnice \ref{eq:gipps}. Její implementace vypadá následovnì:
...
Další funkcí k pøepsání je \textit{stopSpeed}. Tato funkce má stejné parametry jako \textit{followSpeed} až na parametr \textit{predSpeed}, který je v tuto chvíli nulový, protože tato funkce poèítá rychlost, pokud se pøed ní nachazí statický objekt. Proto voláme stejné funkce jako u \textit{followSpeed} s tím, že \textit{predSpeed} má hodnotu nula.
