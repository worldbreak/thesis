...
Hlavní skupinou mikroskopických modelù jsou tzv.~\uv{Car Following} modely. Tyto modely vycházejí z pøedpokladu, že pokud n-té vozidlo následuje $n-1$ vozidlo na homogenní dálnici, trajektorie $n$-tého vozidla bude stejná jako vozidla pøed ním (tedy vozidla $n-1$) až na posuny v èase a místì. (Vozidlo bude ve stejném místì jako pøedcházející v jiný èas a ve stejný èas bude na jiném místì.) Všechny další dispozice jednotlivých modelù v sobì tento pøedpoklad obsahují.

Na konci každého modelu budou pro pøehlednost shrnuty parametry modelu, které se vyskytují v závìreèné rovnici.
...
\label{eq:xntaun}
\end{equation}
kde $T$ znaèí reakèní èas øidièe.
\begin{comment}
...
Další model, kterým se zde budeme zabývat, se jmenuje Gippsùv. Tento model byl poprvé popsán v roce 1981 a odvolává se i na zmínìný Newellùv model. I v dnešní dobì se tento model využívá pro simulaci dopravy. Hlavním zdrojem pro tuto podkapitolu je èlánek P.G. Gippse\cite{Gipps}. Vìtšina modelù (Newellùv ovšem ne), které vznikly pøed Gippsovým, jsou rùzné variace na rovnici
\begin{equation}
a_\n(t+T_n) {}={} l_n\frac{[v_{\nm1}(t) - v_\n(t)]^k}{[x_{\nm1}(t) - x_\n(t)]^m},
...
\end{equation}
%
kde $s_{\nm1}$ je délka vozidla $n-1$, seètena s bezpeènou vzdáleností následujícího ($n$-tého) vozidla. Pokud podmínka \eqref{eq:distance} neplatí, $n$-té vozidlo nemá žádný prostor pro pøípadnou øidièovu chybu. Z toho dùvodu obsahuje Gippsùv model ještì další podmínku: bezpeèný odstup s další èasovou rezervou, $\theta$. Jak již víme, øidiè zaène reagovat na vozidlo pøed sebou až v èase $t+T_\n$. Od této chvíle tedy máme reakèní èas $T_\n$ a bezpeèný reakèní èas $T_\n + \theta$, který budeme ve výpoètech dále využívat. S využitím rovnic \eqref{eq:cometorestnm1} a \eqref{eq:cometorestn} mùžeme podmínku \eqref{eq:distance} pøepsat do nerovnice
%
\begin{multline}
x_{\nm1} (t) - \frac{v_{\nm1} (t)^2}{2b_{\nm1}} - s_{\nm1} \geq  x_\n (t) +\\
+\left[v_\n (t) + v_\n (t+T_n) \right] \frac{T_\n}{2} + v_\n (t+T_n) \theta - \frac{v_{\n} (t+T_\n)^2}{2b}.
...
Relativní hodnoty $T_n$ a $\theta$ jsou dùležité v urèování chování vozidel. Za normálních okolností se dosazuje za $\theta = T_n/2$. Díky tomu mùžeme rovnici \eqref{eq:replacebn} pøepsat do tvaru
\begin{equation}
- \frac{v_{n} (t+T_\n)^2}{2b} + v_\n (t+T_n) T_\n - \left[x_{\nm1} (t) - s_{\nm1} - x_\n (t)\right] + v_{\n} (t) \frac{T_\n}{2} + \frac{v_{\nm1} (t)^2}{2\hat{b}} \leq 0.
...
\begin{itemize}
	\item $T_\n$ - reakèní èas øidièe,
	\item $v_\n (t)$ - rychlost vozidla n v èase $t$,
	\item $a$ - maximální hodnota zrychlení, kterým je øidiè ochoten zrychlovat - \textbf{nikoliv zrychlení vozidla v èase t!},
	\item $V_\n$ - preferovaná rychlost vozidla n,
	\item $b$ - maximální decelerace vozidla, kterým je øidiè ochoten brzdit,
	\item $x_\n$ - poloha vozidla n
	\item $s_{\nm1}$ - délka vozidla $n-1$ seètena s bezpeènou vzdáleností následujícího (n) vozidla
...
\end{comment}
Kraussùv model byl vytvoøen v rámci disertaèní práce stejnojmeného autora. V popisu tohoto modelu budeme vycházet z této práce\cite{Krauss}.
...
Vozidlo je umístìno nad všemi prahy v obrázku \ref{wiedemann} a jede nezávisle na okolní dopravì. Vozidlo jede s maximálním zrychlením tak, aby dosáhlo své preferované rychlosti. Ve chvíli, kdy je dosažena preferovaná rychlost.\rem{doplnit} Nepøesné zacházení s plynem ve vozidle je modelováno pøiøazením akcelerace $b_{null}$ èi $-b_{null}$. Maximální zrychlení je definováno jako
