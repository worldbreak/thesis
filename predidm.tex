...
\subsection{Shrnutí Newellova modelu}
...
Budeme pro zjednodušení uvažovat pouze první 4 èleny Taylorova rozvoje. Potøebujeme rozvinout funkce $f(v_1(t))$ a $f(v_f(t))$ z rovnice \eqref{eq:kraussdvsafe} v okolí bodu $\overline{v}$ z rovnice \eqref{eq:kraussv1vf}. Po vyjadøení funkce $f(v_1(t))$ Taylorovým rozvojem, rovnice \eqref{eq:taylor}, dostáváme
\begin{equation}
\begin{split}
f(v_1(t)) &= f(\overline{v}) + \frac{f^{(1)}(\overline{v})}{1!} \left(v_1(t) - \frac{v_1(t) + v_f(t)}{2}\right) \\
&+ \frac{f^{(2)}(\overline{v})}{2} \left(v_1(t) - \frac{v_1(t) + v_f(t)}{2}\right)^2 + \frac{f^{(3)}(\overline{v})}{6} \left(v_1(t) - \frac{v_1(t) + v_f(t)}{2}\right)^3,
\end{split}
\label{eq:taylorfv1}
\end{equation}
...
\begin{equation}
\begin{split}
f(v_1(t)) &= f(\overline{v}) + \frac{f^{(1)}(\overline{v})}{1!} \left(\frac{v_1(t) - v_f(t)}{2}\right) \\
&+ \frac{f^{(2)}(\overline{v})}{2} \left(\frac{v_1(t) - v_f(t)}{2}\right)^2 + \frac{f^{(3)}(\overline{v})}{6} \left(\frac{v_1(t) - v_f(t)}{2}\right)^3.
\end{split}
\label{eq:taylorfv1}
\end{equation}
...
\begin{equation}
\begin{split}
f(v_f(t)) &= f(\overline{v}) + \frac{f^{(1)}(\overline{v})}{1!} \left(\frac{v_f(t) - v_1(t)}{2}\right) \\
&+ \frac{f^{(2)}(\overline{v})}{2} \left(\frac{v_f(t) - v_1(t)}{2}\right)^2 + \frac{f^{(3)}(\overline{v})}{6} \left(\frac{v_f(t) - v_1(t)}{2}\right)^3.
\end{split}
\label{eq:taylorfvf}
\end{equation}
...
\begin{equation}
f^{(1)}(\overline{v}) \frac{v_f(t) - v_1(t)}{2} + v_f(t) T \leq f^{(1)}(\overline{v}) \frac{v_1(t) - v_f(t)}{2} + g(t).
\label{eq:krausstaylorcompare}
\end{equation}
Po roznásobení a pøevedení èlenù rovnice na opaèné strany získáme
\begin{equation}
f'(\overline{v})v_f + v_f T \leq f'(\overline{v})v_1 + g,
...
\begin{equation}
v_1 - v_f \geq \frac{v_1 T - g}{\frac{\overline{v}}{b(\overline{v})} + T},
...
Nerovnice \eqref{eq:krausssafetycond} se v~podstatì rovná nerovnici \eqref{eq:kraussgapdes}, kde preferovaný odstup $g_{\des} = v_1 t$ a preferovaný bezpeèný èasový odstup mezi vozidly $\tau_{\des} = \tau_b + T$, kde $\tau_b = \overline{v}/b(\overline{v})$ je stanoven na základì decelerací, které øidiè využije.

\subsection{Diskretizace modelu}

Pro možnost simulace na poèítaèi je potøeba pøevést nerovnice v~èásti \ref{krauss:approach} na diskrétní rovnice pro dráhy, rychlosti a zrychlení.
...
\begin{equation}
\xi (t + \Delta t) = \xi(t) + \left(1 - \frac{\Delta t}{\tau_b + T}\right) + \Delta t \frac{g_{\des} - v_1(t)\Delta t}{\tau_b + T},
...
Pøedpokládáme tedy, nezávisle na náhodném kolísání, že se každé vozidlo pohybuje nejvyšší rychlostí na základì podmínek, které jsou stanoveny níže. \textbf{Kraussùv model tedy stanovíme následujícími rovnicemi:}
\begin{equation}
\begin{split}
...
Hlavní zdroj pro vylepšení Kraussova modelu je pøímo èlánek pro implementaci Rozšíøeného Kraussova modelu \cite{Song}. Základem pro toto vylepšení Kraussova modelu plyne z~procesu brždìní.

\subsection{Proces brždìní}

Fáze brzdícího procesu na sebe plynule navazují, první z~nich se nazývá doba reakce øidièe $T$, dále zvyšování decelarace $t_i$ a maximální brždìní $t_c$. Vše je graficky znázornìno na obrázku \ref{brzdeni}. 
...
\paragraph{Doba reakce øidièe:} Øidiè potøebuje urèitý èas $T$ na to, aby zareagoval na vozidlo pøed sebou. V~dobì této reakce se pohybuje rychlost vozidla $v_0$ na konstantní hodnotì, pøípadnì se lehce mìní. Posunutí aktuálního vozidla je potom dán rovnicí
\begin{equation}
S_r = v_0 T.
...
Klíè ke správnému Car-Following modelu je dle Krausse je hlavnì bezpeèná rychlost. Pokud èelní vozidlo sníží rychlost, aktuální vozidlo musí zaèít brzdit. Pokud obì vozidla zastaví, jejich polohu bude možno vypoèíst na základì vztahu
\begin{equation}
S_l + g(t) = S_f,
...
\begin{equation}
S_f = S_r + S_i + S_c = v_{\safe} T + \frac{1}{2} v_{\safe} t_{i} + \frac{v_{\safe}^2}{2b_{\max}} - \frac{b_{\max} t_{fi}^2}{24} \approx v_{\safe} \left(T + \frac{1}{2}t_{fi} \right) + \frac{v_{\safe}^2}{2b_{\max}}.
\label{eq:S_fKrauss}
\end{equation}
...
\begin{equation}
v_{\safe} = -b \left(T + \frac{t_{fi}}{2}\right) + \sqrt{b^2 \left(t_r + \frac{t_{fi}}{2}\right)^2 + b\left(v_l t_{li} + \frac{v_l^2}{a} + 2g\right)}.
\label{eq:vsafeKrauss}
\end{equation}

V~simulaci je potom èas decelerace stejný $t_i = t_{li} = t_{fi}$, z~toho plyne
\begin{equation}
v_{\safe} = -b \left(T + \frac{t_{i}}{2}\right) + \sqrt{b^2 \left(T + \frac{t_{i}}{2}\right)^2 + b\left(v_l t_{i} + \frac{v_l^2}{a} + 2g\right)},
\label{eq:vsafeKraussti}
\end{equation}
pokud èelní vozidlo stojí, je jeho rychlost $v_l = 0 \text{ms}^{-1}$ a jeho poloha se nemìní $S_l = 0$, potom mùžeme zjednodušit rovnici \eqref{eq:vsafeKraussti} na
\begin{equation}
v_{\safe} = -b \left(T + \frac{t_{i}}{2}\right) + \sqrt{b^2 \left(T + \frac{t_{i}}{2}\right)^2 + 2bg}.
\label{eq:newKrauss}
\end{equation}

Tato rovnice \eqref{eq:newKrauss} je  jakýmsi doplòkem ke Kraussovu modelu, pouze pøesnìji definuje bezpeènou rychlost. Princip Kraussova modelu stále zùstává ve volbì preferované rychlosti, kterou se vozidlo bude pohybovat. Tato volba je dána minimální hodnotou, viz rovnice $v_{\des}$ v~\eqref{eq:krausssmodel}.
...
\label{sec:IDM}
Intelligent Driver Model (dále jen IDM) je pomìrnì nový matematický model z~roku 2000, který byl vymyšlen v~Nìmecku. Snaží se vylepšit nìkteré nedostatky pøedcházejících modelù, ale i pøispìt novými myšlenkami v~této oblasti.

V~této sekci budeme primárnì vycházet z~práce \cite{Vanis} a \cite{TreiberWWW}, ve které je IDM do znaèné míry rozebrán. Využijeme hlavnì podkapitolu 5.1 z~\cite{Vanis}, kterou doplníme o~poznatky, které jsou uplatnìny z~kapitoly \ref{chap:predIDM}.
...
\label{soucetzrych}
\end{equation}
kde $v_\n (t)$ znaèí rychlost vozidla v~èase $t$ a $\Delta T$ je simulaèní krok. Aktuální zrychlení vozidla se tedy vypoèítá jako souèet zrychlení pøi volném proudu $a_\free$ a zrychlení, ve kterém je zahrnut i vozidlo jedoucí pøed ním $a_\inter$. Zde je vidìt urèitá paralela s~Gippsovým modelem (kapitola \ref{sec:Gipps}), kde se ovšem vybírá pouze jedna z~tìchto dvou hodnot. Navíc i rovnice se od IDM odlišují. Pro volný proud je tedy zrychlení $a_\inter {}={} 0$.
...
\begin{equation}
s_n^* = s_\text{0} + v_n \tau + \frac{v_n(v_n - v_{n-1})}{2\sqrt{ab}}
\label{snhvez}
\end{equation}
je preferovaný odstup. Hodnota $s_\text{0}$ se vypoète jako souèet minimální bezpeèné vzdálenosti mezi vozidly a délkou následujícího vozidla, $\tau$ je bezpeènostní èasový odstup mezi vozidly a $b$ je konstanta brzdìní. Situace je schématicky znázornìna na obrázku \ref{promen}.
...
Dosazením do rovnice \eqref{soucetzrych} za $a_\inter$ z~rovnice \eqref{intclen} a $a_\free$ z~rovnice \eqref{zrychvolproud} dostáváme \textbf{koneèný vztah pro zrychlení vozidla}:
\begin{equation}
v_{\n}(t + \Delta T) = v_\n (t) + a\left[1-\left(\frac{v_n}{v_{n\free}}\right)^\delta\right] - a \left(\frac{s_\text{0} + v_n \tau + \frac{v_n(v_n - v_{n-1})}{2\sqrt{ab}}}{s_n}\right)^2.
\label{eq:IDMcompl}
\end{equation}
...
\begin{itemize}
	...
	\item $\tau$ - èasová mezera mezi vozidly,
