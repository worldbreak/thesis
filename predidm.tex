...
\subsection{Simulaèní model vycházející z Kraussova modelu}
...
V této sekci budeme primárnì vycházet z práce \cite{Vanis} a \cite{TreiberWWW}, ve které je Intelligent Driver Model (IDM) do znaèné míry rozebrán. Využijeme hlavnì podkapitolu 5.1 z \cite{Vanis}, kterou doplníme o poznatky, které jsou uplatnìny z kapitoly \ref{chap:predIDM}.
...
Tento model je odlišný oproti pøedcházejícím. Pracuje totiž v tzv. režimech. Nejprve je nutné zjistit, v jakém režimu se vozidlo vlastnì nachází a podle toho se urèuje výpoèet akcelerace èi decelarace, která je pro každý režim jiná. 
...
Parametry $EXadd$ a $EXmult$ jsou opìt parametry pro zpøesnìní výpoètù, hodnota $NRND$ je generována z normálního rozdìlení a $RND2_n$ znaèí parametr závislý na øidièi generovaný také z normálního rozdìlení.
...
Hodnota zrychlení vozidla je vždy poèítána každou iteraci znovu od nuly. Dùvodem toho je nepøesné zacházení s plynovým pedálem ve vozidle. Pokud vozidlo pøekoná práh $SDV$ èi $ABX$, jeho hodnota akcelerace bude rovna $- b_{null}$. Naopak pøi dosáhnutí prahù $OPDV$ èi $SDX$ bereme kladnou hodnotu $b_{null}$. Akcelerace èi decelerace je definována jako
\begin{equation}
b_{null} = BNULLmult \cdot (RND4_n + NRND),
\label{eq:bnull}
\end{equation}

kde $BNULLmult$ je parametr modelu, hodnota $NRND$ je generována z normálního rozdìlení a $RND4_n$ znaèí parametr závislý na øidièi generovaný také z normálního rozdìlení.

\paragraph{Režim plné rychlosti:}
Vozidlo je umístìno nad všemi prahy v obrázku \ref{wiedemann} a jede nezávisle na okolní dopravì. Vozidlo jede s maximálním zrychlením tak, aby dosáhlo své preferované rychlosti. Ve chvíli, kdy je dosažena preferovaná rychlost. Nepøesné zacházení s plynem ve vozidle je modelováno pøiøazením akcelerace $b_{null}$ èi $-b_{null}$. Maximální zrychlení je definováno jako
...
Kromì Newellova modelu a dalších modelù pro základní výzkum existují také vysoce komplexní modely jako Wiedemannùv (viz. odstavec \ref{sec:Wiedemann}). Ten se snaží napodobovat dopravu jak nejlépe to jde ovšem za cenu mnoha parametrù modelu.
