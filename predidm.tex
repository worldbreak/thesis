...
V teorii dopravního proudu je Newellùv model jeden ze základních modelù popisujících chování øidièe \rem{vozidla?} dle vozidla pøed ním. Øidiè se snaží držet za vozidlem jedoucím pøed ním v konstantní  vzdálenosti. Tento model vznikl v roce 1961 a postupnì se vyvíjel \rem{vyvíjel se v co?}. Zde jej zmiòujeme hlavnì z dùvodu, že z Newellových úvah vychází mnohé novìjší modely, napøíklad \emph{Intelligent Driver Model} (IDM), který si popíšeme pozdìji.
...
\begin{figure}[ht]%
    ...
    \newline\rem{sneslo by pøekreslit do TikZu}
...
\label{eq:xntaun}
\end{equation}
kde $T_\n$ znaèí reakèní èas øidièe. \rem{Doplnit}
...
\begin{itemize}
	...
	\item parametry modelu by mìly odpovídat vlastnostem øidièe a vozidla, které jsou jasné; to znamená, že pro nastavení parametrù není potøeba model kalibrovat, \rem{Bacha na to}
...
\end{equation}
%
kde èlen $b_{\nm1}$ oznaèuje nejvyšší kritické zpomalení \rem{decelarace?}, kterou je øidiè vozidla $n-1$ schopen vykonat. Její hodnota je vždy záporná.
...
Relativní hodnoty $T_n$ a $\theta$ jsou dùležité v urèování chování vozidel. Stejnì jako v pøípadì Newellova modelu, kapitola \ref{sec:Newell}, rovnice \eqref{eq:Tn}, pokud je hodnota $\theta$ \footnote{ekvivalentní s $\tau_{\n}$} rovna právì jedné polovinì $T_\n$, vozidlo jedoucí bezpeènou rychlostí bude schopno udržovat tento stav nekoneènì dlouhou dobu. Díky tomu mùžeme rovnici \eqref{eq:replacebn} pøepsat do tvaru
\begin{equation}
- \frac{v_{n} (t+T_\n)^2}{2b} + v_\n (t+T_n) T_\n - \left[x_{\nm1} (t) - s_{\nm1} - x_\n (t)\right] + v_{\n} (t) \frac{T_\n}{2} + \frac{v_{\nm1} (t)^2}{2\hat{b}} \leq 0.
...
\begin{itemize}
	\item $t$ - aktuální èas
	\item $T_\n$ - reakèní èas, stejný pro všechna vozidla,
...
\label{sec:Krauss}
Pro simulaci v této práci budeme využívat software SUMO\footnote{Simulation of Urban MObility} a souèástí tohoto programu je pøedpøipravený Kraussùv model vytvoøený v rámci disertaèní práce stejnojmeného autora. V popisu tohoto modelu budeme vycházet z této práce\cite{Krauss}. \rem{preformulovat}
...
Místo toho zavedeme pravidlo, že se v spojitých modelech spolu interagující vozidla nesrazí tehdy, pokud je mezera mezi vozidly $g$ vìtší než nìjaká preferovaná mezera mezi vozidly $g_{\des}$ a splòuje tuto dynamickou nerovnici
\begin{equation}
...
Nerovnice \eqref{eq:krausssafetycond} se v podstatì rovná nerovnici \eqref{eq:kraussgapdes}, kde preferovaný odstup $g_{\des} = v_1 t$ a preferovaný bezpeèný èasový odstup mezi vozidly $\tau_{\des} = \tau_b + \tau$, kde $\tau_b = \overline{v}/b(\overline{v})$ je stanoven na základì decelerací, které øidiè využije.
...
\end{split}
\end{equation}
To je výsledek, který se ovšem dal pøedpokládat. Záleží pouze na tom, zda je i po provedení iteraèního kroku stále splnìna podmínka bezpeènosti, tzn. kontrolovat jestli je náš iteraèní krok menší než reakèní èas øidièe $\tau$.
